...
export const subscriptionNudges = schedules.task({
  ...
  run: async () => {
    ...
    if (!subscriptions || subscriptions.length === 0) return { nudged: 0 };
    ...
    for (const sub of subscriptions) {
      ...
      const subscriber = Array.isArray(sub.subscriber)
        ? sub.subscriber[0] : sub.subscriber;
      const creator = Array.isArray(sub.creator)
        ? sub.creator[0] : sub.creator;
      ...
      await resend.emails.send({
        ...
        html: `
          ...
          ${isNonRenewing
            ? `<p>Your subscription to <strong>${plan?.title}</strong> by <strong>${creator?.full_name}</strong> ends on <strong>${expiryDate}</strong> and will not renew.</p>
               <p>To resubscribe visit their <a href="https://campuswhop.com/creators">creator page</a>.</p>`
            : `<p>Your subscription to <strong>${plan?.title}</strong> by <strong>${creator?.full_name}</strong> renews on <strong>${expiryDate}</strong> for <strong>₦${plan?.price?.toLocaleString()}</strong>.</p>
