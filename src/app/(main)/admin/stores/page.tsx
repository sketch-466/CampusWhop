...
export default async function AdminStoresPage() {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h2 className="text-lg font-bold text-white">Stores & Products</h2>
        <p className="text-sm text-zinc-400 mt-0.5">
          {stores.length} store{stores.length !== 1 ? 's' : ''} · {products.length} product{products.length !== 1 ? 's' : ''} pending review.
        </p>
      </div>
          ...
          stores.map((store: any) => (
            <div key={store.id} className="overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900">
              <div className="p-4 space-y-1">
                <h4 className="text-sm font-bold text-zinc-100">{store.store_name}</h4>
            ...
            <div key={product.id} className="overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900">
              ...
              <div className="flex gap-2 border-t border-zinc-800 px-4 py-3">
                <form action={async () => {
                  'use server'
                  const adminClient = createAdminClient()
                  await adminClient.from('store_products')
                    .update({ status: 'active', updated_at: new Date().toISOString() })
                    .eq('id', product.id)
                  revalidatePath('/admin/stores')
                }}>
