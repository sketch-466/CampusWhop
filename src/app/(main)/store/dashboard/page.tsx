    ...
    <div className="mx-auto max-w-5xl px-4 py-6">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          <Link href="/dashboard" className="inline-flex items-center gap-1 text-sm text-zinc-400 hover:text-white">
            <ArrowLeft className="h-4 w-4" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold text-white">{store.store_name}</h1>
            <div className="flex items-center gap-2 mt-1">
              <Badge variant={statusColors[store.status] || "default"}>{store.status}</Badge>
      ...
      {error ? (
        ...
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/30 p-12 text-center">
          ...
          <Link href="/store/dashboard/new">
            <Button className="mt-4 bg-emerald-500 hover:bg-emerald-600">Add Your First Product</Button>
          </Link>
        </div>
      ) : (
          ...
          {products.map((product: any) => (
            <div key={product.id} className="relative">
              ...
              <div className="absolute top-2 left-2">
                <Badge variant={statusColors[product.status] || "default"} className="text-xs">
                  {product.status}
                </Badge>
              </div>
            </div>
