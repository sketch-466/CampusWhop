...
import {
  Briefcase, ShoppingBag, Store, Home,
  GraduationCap, ArrowRight, Plus, Shield,
} from 'lucide-react'
...
export default async function DashboardPage() {
  ...
  const [{ data: subaccount }, { data: reputationData }] = await Promise.all([
    supabase
        ...
        <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
          {quickActions.map((action) => (
            ...
            >
              ...
              <h4 className="mt-2 text-sm font-medium text-white">{action.title}</h4>
