...
export default async function AdminFeaturedPage() {
  ...
  async function featureListing(formData: FormData) {
    ...
    await adminClient
      .from("listings")
      .update({ is_featured: true, featured_until: until.toISOString(), updated_at: new Date().toISOString() })
      .eq("id", id);
      ...
      <section className="space-y-3">
        ...
        <div className="space-y-2">
          {(activeListings || []).map((listing: any) => {
            const isFeatured = listing.is_featured && listing.featured_until && listing.featured_until > now;
            const seller = Array.isArray(listing.profiles) ? listing.profiles[0] : listing.profiles;

            return (
              <div key={listing.id}
                className={`flex items-center gap-3 rounded-xl border p-3 ${
                  isFeatured ? "border-amber-800/40 bg-amber-900/10" : "border-zinc-800 bg-zinc-900/30"
                }`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5">
                    ...
                    {isFeatured && <Star className="h-3 w-3 text-amber-400 fill-amber-400 shrink-0" />}
                  </div>
                  <p className="text-xs text-zinc-500">
                    ₦{listing.price?.toLocaleString()} · {seller?.full_name ?? "Unknown"}
                  </p>
                  {isFeatured && listing.featured_until && (
                    <p className="text-xs text-amber-500/70">
                      Until {new Date(listing.featured_until).toLocaleDateString("en-NG", {
      ...
      <section className="space-y-3">
        ...
        <div className="space-y-2">
          {(activeProducts || []).map((product: any) => {
            const isFeatured = product.is_featured && product.featured_until && product.featured_until > now;
            ...
            return (
              <div key={product.id}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-zinc-500">
                    ₦{product.price?.toLocaleString()} · {store?.store_name ?? "Unknown"}
                  </p>
                  {isFeatured && product.featured_until && (
                    <p className="text-xs text-amber-500/70">
                      Until {new Date(product.featured_until).toLocaleDateString("en-NG", {
                        day: "numeric", month: "short", year: "numeric"
                      })}
                    </p>
                  )}
